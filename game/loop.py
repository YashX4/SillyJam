...
async def run() -> None:
    ...
    state.gear_sprite = gear.load_gear_sprite()
    state.selection_sprite = selection.load_selection_sprite()
    dt_accumulator = 0.0
    ...
    while not rl.window_should_close():
        ...
        if state.gear_sprite is not None:
            state.gear_sprite.update(rl.get_frame_time())
        if state.selection_sprite is not None:
            state.selection_sprite.update(rl.get_frame_time())
